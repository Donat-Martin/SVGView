...
class SVGTextParser: SVGBaseElementParser {
  override func doParse(context: SVGNodeContext, delegate: (XMLElement) -> SVGNode?) -> SVGNode? {
    ...
    print(context.element.contents)
    let text = context.element.contents.compactMap({ ($0 as? XMLText)?.text })
      .joined(separator: "")
      .trimmingCharacters(in: .whitespacesAndNewlines)
      .processingWhitespaces()
    if !text.isEmpty {
    // if let textNode = context.element.contents.first as? XMLText {
      ...
      return SVGText(text: text, font: font, fill: SVGHelper.parseFill(context.styles, context.index), stroke: SVGHelper.parseStroke(context.styles, index: context.index), textAnchor: textAnchor, transform: transform)
    }
    return .none
  }
